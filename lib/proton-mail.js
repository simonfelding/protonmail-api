...
class ProtonMail {
  ...
  async _connect () {
    if (this._browser === undefined) {
      this._browser = await chromium.puppeteer.launch({ headless: true, ...this._config.puppeteerOpts })
   ...
   * @param {Address|string} options.from_name
   * @param {Address|string} options.from_mail
   * @param {string} options.subject
   * @param {string} options.body
   * @return {Email}
   */
  async sendEmail (options) {
    const { to, subject, body, from_name, from_mail } = options
    const data = {
    ...
    const emailId = await this._page.evaluate(async (data) => {
      ...
      const draftData = {
        Message: {
          ...
          BCCList: [],
          Unread: 0,
          Sender: { Name: data.from_name || '', Address: data.from_mail }
        }
      }
